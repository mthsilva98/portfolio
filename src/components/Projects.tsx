...
// IMPORTS das imagens (funcionam em produção)
import mentalHealthImg from '@/assets/mental-health-platform.svg';
import ifoodImg from '@/assets/ifood-backoffice.svg';
import fintechImg from '@/assets/fiap-fintech.svg';
import extracaoImg from '@/assets/extracao.svg';
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            {projects.map((project, index) => (
              ...
              >
                ...
                {/* Action Buttons (desabilita quando não há link) */}
                <div className="flex space-x-3 mt-6">
                  {project.liveUrl ? (
                    <Button className="flex-1 gradient-primary text-white hover:shadow-glow" asChild>
                    ...
                    <Button className="flex-1 gradient-primary text-white" disabled>
                      <ExternalLink className="mr-2 h-4 w-4" />
                      {t.viewProject}
                    </Button>
                  )}
                  ...
                  {project.githubUrl ? (
                    <Button variant="outline" className="flex-1 border-primary text-primary hover:bg-primary hover:text-white" asChild>
                      <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center">
                        <Github className="mr-2 h-4 w-4" />
                        {t.viewCode}
                      </a>
                    </Button>
                  ) : (
                    <Button variant="outline" className="flex-1 border-primary text-primary" disabled>
                      <Github className="mr-2 h-4 w-4" />
                      {t.viewCode}
                    </Button>
                  )}
